...
function MetricCard({ icon: Icon, title, metric, detail, delay }: MetricCardProps) {
  ...
  return (
    <motion.div 
      ...
      className="relative p-6 sm:p-8 rounded-2xl bg-slate-900/40 border border-slate-800 hover:border-cyan-500/50 transition-all cursor-pointer overflow-hidden flex flex-col justify-center group"
    >
    ...
    <main className="min-h-screen bg-[#050811] text-slate-100 font-sans selection:bg-cyan-500 selection:text-black overflow-x-hidden">
      ...
      <section className="relative z-20 py-24 px-4 sm:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <MetricCard 
            icon={Activity} 
            title="Fluid Barrier Rating" 
            metric="2,500 PSI" 
            delay={0.2}
            detail={
              <ul className="space-y-1.5 list-disc pl-4 text-slate-300">
                <li><strong className="text-cyan-400">Leak Rate:</strong> 1x10⁻⁹ atm cc/sec (Helium test)</li>
      ...
      <section className="relative z-20 py-24 px-4 sm:px-8 bg-slate-900/20 border-y border-slate-800/80">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-white">Scale Without Broker Friction</h2>
            <p className="text-slate-400 text-sm">Direct contract manufacturing vs legacy brokerage supply chains.</p>
          </motion.div>

          <motion.div 
            initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="overflow-x-auto rounded-2xl border border-slate-800 bg-slate-950/60 backdrop-blur-sm"
          >
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-slate-900/90 border-b border-slate-800">
                <tr>
                  <th className="p-6 text-slate-400 font-mono text-xs uppercase tracking-widest">Procurement Dimension</th>
                  <th className="p-6 text-cyan-400 font-bold border-x border-cyan-900/50 bg-cyan-950/20 text-base">Syntrel Direct Hub</th>
                  <th className="p-6 text-slate-500 text-base font-normal">Legacy Brokerages</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60 text-base">
                <tr>
                  <td className="p-6 font-medium text-white flex items-center gap-3"><Clock className="w-5 h-5 text-cyan-500" /> Rapid Pilot Dispatch</td>
                  <td className="p-6 font-bold text-white bg-cyan-950/10 border-x border-cyan-900/50">3 - 5 Days</td>
                  <td className="p-6 text-slate-500">21 - 45 Days</td>
                </tr>
                <tr>
                  <td className="p-6 font-medium text-white flex items-center gap-3"><Globe2 className="w-5 h-5 text-cyan-500" /> Customs Tariffs</td>
                  <td className="p-6 font-bold text-emerald-400 bg-cyan-950/10 border-x border-cyan-900/50">0% (CEPA Duty-Free)</td>
                  <td className="p-6 text-slate-500">5% - 15% Customs Duty</td>
                </tr>
                <tr>
                  <td className="p-6 font-medium text-white flex items-center gap-3"><ShieldCheck className="w-5 h-5 text-cyan-500" /> Cost Transparency</td>
                  <td className="p-6 font-bold text-white bg-cyan-950/10 border-x border-cyan-900/50">Open-Book Factory Invoicing</td>
                  <td className="p-6 text-slate-500">25%+ Opaque Markups</td>
                </tr>
              </tbody>
            </table>
          </motion.div>
      ...
      <section id="rfq" className="relative z-20 py-24 px-4 sm:px-8 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-cyan-950/20 via-[#050811] to-[#050811]">
        <motion.div 
          ...
          className="max-w-4xl mx-auto rounded-3xl border border-cyan-900/50 bg-[#070b14] overflow-hidden shadow-[0_0_80px_rgba(6,182,212,0.06)]"
        >
          ...
          <div className="p-8 sm:p-14 text-center">
            ...
            <form className="space-y-4 max-w-md mx-auto" onSubmit={(e) => e.preventDefault()}>
              <input 
                ...
                className="w-full px-5 py-4 rounded-xl bg-slate-950 border border-slate-700 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner text-sm" 
                required 
              />
